# 08: PRISMA, PRESS and reporting

## Objective

Generate reporting artefacts from audit evidence and separate conduct checks from reporting checks.

## Scope

- Complete 16-item PRISMA-S evidence mapping
- Implement PRISMA 2020 new/update flow variants
- Add PRISMA-ScR and PRISMA-LSR profiles
- Create six-domain PRESS review contract and workflow
- Render Markdown, JSON, Mermaid, SVG, Typst and DOCX-friendly tables
- Establish PRISMA.jl parity then choose canonical ownership

## Requirements owned

See `conductor/requirements.md` rows whose Track owner includes `08`.

## Acceptance contract

- All named tasks have implementation, deterministic tests, documentation and a
  machine-readable verification receipt or an explicit external blocker.
- Public claims remain at the achieved evidence level.
- Security, privacy, migration and rollback impact are reviewed.
- CLI/MCP/facade parity is preserved where the track changes a public operation.

## Out of scope

Work owned by later tracks is documented but not promoted as implemented.
