<!-- searchright-issue-key: track-17-phase-1 -->
# Track 17 / Phase 1: Source implementation

Parent track key: `track-17`
Conductor plan: `conductor/tracks/17-benchmarks-calibration/plan.md`

## Task subissues

- [ ] T01: Complete every acceptance assertion; existing paths are scaffolding or partial implementation only. (`track-17-phase-1-task-01`)

## Evidence rule

Remote completion is a planning signal only. Evidence is promoted only through the track evidence record and a reproducible receipt at the claimed level.
