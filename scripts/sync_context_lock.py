...
def canonical_bytes(p:Path)->bytes:
 data=p.read_bytes()
 return data if b'\0' in data[:8000] else data.replace(b'\r\n',b'\n')
def digest(p:Path)->str: return hashlib.sha256(canonical_bytes(p)).hexdigest()
def build()->dict:
 m=json.loads(MANIFEST.read_text()); paths=[ROOT/x['path'] for x in m['required_context']]
 return {'schema_version':'org.searchright.context-lock.v1','generated_at':'source-epoch:2026-08-09','files':[{'path':p.relative_to(ROOT).as_posix(),'sha256':digest(p),'size':len(canonical_bytes(p))} for p in sorted(paths)],'claim_boundary':'Hash parity detects context drift; it does not establish semantic correctness or promote evidence.'}
def main()->int:
