...
def build()->dict:
 ...
 return {'schema_version':'org.searchright.context-lock.v1','generated_at':'2026-08-06','files':[{'path':p.relative_to(ROOT).as_posix(),'sha256':digest(p),'size':p.stat().st_size} for p in sorted(paths)],'claim_boundary':'Hash parity detects context drift; it does not establish semantic correctness or promote evidence.'}
def main()->int:
